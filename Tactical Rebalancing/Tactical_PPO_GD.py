...
def initialize(context):
    ...
    context.date = datetime.datetime(2008, 5, 27)
...
def rebalance(context,data):
    ...
    context.ppo_daywise.append(portfolio_ppo)
    " RE-adjust the portfolio if there is a trend goes up"
    # Taking second gradient for the macd
    ppo_gradient = np.gradient(np.gradient(np.array(context.ppo_daywise)))     
    if( ppo_gradient[ppo_gradient.size-1] > 0.2 and 
        context.portfolio.cash < context.portfolio.portfolio_value*0.30):
        # if portfolio_ppo < 0 or (not(math.isnan(portfolio_macd) and portfolio_macd < -1.0)):
        for security in context.long_portfolio:
            order_value(security, -1*(context.portfolio.portfolio_value*0.1)/len(context.long_portfolio))
    if(ppo_gradient[ppo_gradient.size-1] < -0.2 and
        context.portfolio.cash > context.portfolio.portfolio_value*0.30) :
        for security in context.long_portfolio:
            order_value(security, (context.portfolio.portfolio_value*0.1)/len(context.long_portfolio))
    # increment date
